...
const SessionName = "_goat_session"

// Store can/should be set by applications using goat.c. The default is a cookie store.
...
See https://github.com/haatos/goat/blob/master/examples/main.go to see this in action.
*/
...
var SetState = func(r *http.Request) string {
	...
	if err != nil {
		panic("goat.c: source of randomness unavailable: " + err.Error())
	}
...
var GetState = func(r *http.Request) string {
	params := r.URL.Query()
...
func GetAuthURL(w http.ResponseWriter, r *http.Request) (string, error) {
	if !keySet && defaultStore == Store {
		fmt.Println(
			"goat.goat.c: no SESSION_SECRET environment variable is set. The default cookie store is not available and any calls will fail. Ignore this warning if you are using a different store.",
		)
	}

	providerName, err := GetProviderName(r)
	if err != nil {
		return "", err
	}

	provider, err := goat.GetProvider(providerName)
	if err != nil {
		return "", err
	}
	sess, err := provider.BeginAuth(SetState(r))
	if err != nil {
		return "", err
	}
	...
	err = StoreInSession(providerName, sess.Marshal(), w, r)
...
See https://github.com/haatos/goat/blob/master/examples/main.go to see this in action.
*/
var CompleteUserAuth = func(w http.ResponseWriter, r *http.Request) (goat.User, error) {
	if !keySet && defaultStore == Store {
		fmt.Println(
			"goat.goat.c: no SESSION_SECRET environment variable is set. The default cookie store is not available and any calls will fail. Ignore this warning if you are using a different store.",
		)
	...
	value, err := GetFromSession(providerName, r)
	if err != nil {
		return goat.User{}, err
	}
	defer Logout(w, r)
...
func validateState(r *http.Request, sess goat.Session) error {
...
func GetContextWithProvider(r *http.Request, provider string) *http.Request {
	return r.WithContext(context.WithValue(r.Context(), ProviderParamKey, provider))
}

// StoreInSession stores a specified key/value pair in the session.
func StoreInSession(key string, value string, w http.ResponseWriter, r *http.Request) error {
