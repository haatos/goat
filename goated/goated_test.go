...
var fauxProvider goat.Provider

func init() {
	Store = NewProviderStore()
	fauxProvider = &faux.Provider{}
	goat.UseProviders(fauxProvider)
}
...
func Test_BeginAuthHandler(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("error getting faux Goatic session: %v", err)
	}

	sessStr, ok := sess.Values["faux"].(string)
	if !ok {
		t.Fatalf("Goatic session not stored as marshalled string; was %T (value %v)",
			sess.Values["faux"], sess.Values["faux"])
	}
	goatedSession, err := fauxProvider.UnmarshalSession(ungzipString(sessStr))
	if err != nil {
		t.Fatalf("error unmarshalling faux Goatic session: %v", err)
	}
	au, _ := goatedSession.GetAuthURL()
