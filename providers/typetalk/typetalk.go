...
// Provider is the implementation of `goat.Provider` for accessing Typetalk.
...
func (p *Provider) FetchUser(session goat.Session) (goat.User, error) {
	...
	response, err := p.Client().
		Get(endpointProfile + "?access_token=" + url.QueryEscape(sess.AccessToken))
	if err != nil {
		if response != nil {
			response.Body.Close()
		}
	...
	if response.StatusCode != http.StatusOK {
		return user, fmt.Errorf(
			"%s responded with a %d trying to fetch user name",
			p.providerName,
			response.StatusCode,
		)
	}
	...
	response, err = p.Client().
		Get(endpointUser + u.Account.Name + "?access_token=" + url.QueryEscape(sess.AccessToken))
	if err != nil {
		if response != nil {
			response.Body.Close()
		}
	...
	if response.StatusCode != http.StatusOK {
		return user, fmt.Errorf(
			"%s responded with a %d trying to fetch profile",
			p.providerName,
			response.StatusCode,
		)
	}
