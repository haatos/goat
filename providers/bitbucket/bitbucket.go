...
// Provider is the implementation of `goat.Provider` for accessing Bitbucket.
...
func (p *Provider) Client() *http.Client {
	return goat.HTTPClientWithFallBack(p.HTTPClient)
}
...
func (p *Provider) FetchUser(session goat.Session) (goat.User, error) {
	...
	if user.AccessToken == "" {
		...
		return user, fmt.Errorf(
			"%s cannot get user information without accessToken",
			p.providerName,
		)
	}
...
func (p *Provider) getUserInfo(user *goat.User, sess *Session) error {
	...
	if response.StatusCode != http.StatusOK {
		return fmt.Errorf(
			"%s responded with a %d trying to fetch user information",
			p.providerName,
			response.StatusCode,
		)
	}
...
func (p *Provider) getEmail(user *goat.User, sess *Session) error {
	...
	if response.StatusCode != http.StatusOK {
		return fmt.Errorf(
			"%s responded with a %d trying to fetch email addresses",
			p.providerName,
			response.StatusCode,
		)
	}
...
func authenticateRequest(r *http.Request, sess *Session) {
	r.Header.Add("Authorization", "Bearer "+sess.AccessToken)
}
...
func (p *Provider) RefreshToken(refreshToken string) (*oauth2.Token, error) {
	...
	ts := p.config.TokenSource(goat.ContextForClient(p.Client()), token)
