...
var (
	...
	ErrNoVerifiedGitHubPrimaryEmail = errors.New(
		"The user does not have a verified, primary email address on GitHub",
	)
)
...
func NewCustomisedURL(
	clientKey, secret, callbackURL, authURL, tokenURL, profileURL, emailURL string,
	scopes ...string,
) *Provider {
	p := &Provider{
		ClientKey:    clientKey,
...
// Provider is the implementation of `goat.Provider` for accessing Github.
...
func (p *Provider) FetchUser(session goat.Session) (goat.User, error) {
	...
	if response.StatusCode != http.StatusOK {
		return user, fmt.Errorf(
			"GitHub API responded with a %d trying to fetch user information",
			response.StatusCode,
		)
	}
...
func userFromReader(reader io.Reader, user *goat.User) error {
	u := struct {
		ID       int    `json:"id"`
...
func getPrivateMail(p *Provider, sess *Session) (email string, err error) {
	...
	if response.StatusCode != http.StatusOK {
		return email, fmt.Errorf(
			"GitHub API responded with a %d trying to fetch user email",
			response.StatusCode,
		)
	}
