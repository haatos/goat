...
// Provider is the implementation of `goat.Provider` for accessing Gitlab.
...
func NewCustomisedURL(
	clientKey, secret, callbackURL, authURL, tokenURL, profileURL string,
	scopes ...string,
) *Provider {
	p := &Provider{
		ClientKey:    clientKey,
...
func (p *Provider) FetchUser(session goat.Session) (goat.User, error) {
	...
	response, err := p.Client().
		Get(p.profileURL + "?access_token=" + url.QueryEscape(sess.AccessToken))
	if err != nil {
		if response != nil {
			response.Body.Close()
		}
