...
func NewCustomisedURL(
	clientKey, secret, callbackURL, authURL, tokenURL, userAPIEndpoint string,
	scopes ...string,
) *Provider {
	p := &Provider{
		ClientKey:       clientKey,
...
// Provider is the implementation of `goat.Provider` for accessing Influx.
...
func (p *Provider) FetchUser(session goat.Session) (goat.User, error) {
	...
	response, err := p.Client().
		Get(p.UserAPIEndpoint + "?access_token=" + url.QueryEscape(sess.AccessToken))
