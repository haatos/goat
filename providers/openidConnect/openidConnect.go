...
// Provider is the implementation of `goat.Provider` for accessing OpenID Connect provider
...
func (p *Provider) FetchUser(session goat.Session) (goat.User, error) {
	...
	if sess.IDToken == "" {
		return goat.User{}, fmt.Errorf("%s cannot get user information without id_token", p.providerName)
	}
	...
	if err != nil {
		return goat.User{}, fmt.Errorf("oauth2: error decoding JWT token: %v", err)
	}

	expiry, err := p.validateClaims(claims)
	if err != nil {
		return goat.User{}, fmt.Errorf("oauth2: error validating JWT token: %v", err)
	}
...
func (p *Provider) userFromClaims(claims map[string]interface{}, user *goat.User) {
	// required
