...
import (
	...
	"github.com/haatos/goat/providers/lark"
...
func (m *MockedHTTPClient) RoundTrip(r *http.Request) (*http.Response, error) {
...
func Test_GetAppAccessToken(t *testing.T) {
	t.Run("happy path", func(t *testing.T) {
		...
		mockClient.On("RoundTrip", mock.Anything).Return(&http.Response{
			...
			Body: io.NopCloser(
				strings.NewReader(
					`{"code":0,"msg":"ok","app_access_token":"test_token","expire":3600}`,
				),
			),
		}, nil)
	...
	t.Run("error on request", func(t *testing.T) {
		...
		mockClient.On("RoundTrip", mock.Anything).
			Return(&http.Response{}, errors.New("request error"))
...
func Test_FetchUser(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		...
		mockClient.On("RoundTrip", mock.Anything).Return(&http.Response{
			...
			Body: io.NopCloser(
				strings.NewReader(
					`{"code":0,"msg":"ok","data":{"user_id":"test_user_id","name":"test_name","avatar_url":"test_avatar_url","enterprise_email":"test_email"}}`,
				),
			),
		}, nil)
