...
// Provider is the implementation of `goat.Provider` for accessing TikTok
...
func (p *Provider) GetClient() *http.Client {
	return goat.HTTPClientWithFallBack(p.Client)
}
...
func (p *Provider) FetchUser(session goat.Session) (goat.User, error) {
	...
	if user.AccessToken == "" || user.UserID == "" {
		return user, fmt.Errorf(
			"%s cannot get user information without accessToken and userID",
			p.providerName,
		)
	}
