...
// Provider is the implementation of `goat.Provider` for accessing Amazon.
...
func (p *Provider) FetchUser(session goat.Session) (goat.User, error) {
	...
	response, err := goat.HTTPClientWithFallBack(p.Client()).Get(endpointProfile + "?access_token=" + url.QueryEscape(sess.AccessToken))
