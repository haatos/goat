...
// Provider is the implementation of `goat.Provider` for accessing AWS Cognito.
...
func NewCustomisedURL(
	clientID, secret, callbackURL, authURL, tokenURL, issuerURL, profileURL string,
	scopes ...string,
) *Provider {
	p := &Provider{
		ClientKey:    clientID,
...
func (p *Provider) Debug(debug bool) {
	if debug {
		fmt.Println("WARNING: Debug request for goat.providers/cognito but no debug is available")
	}
}
...
func userFromReader(r io.Reader, user *goat.User) error {
	u := struct {
		ID            string `json:"sub"`
