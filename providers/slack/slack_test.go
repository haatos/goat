...
func Test_Implements_Provider(t *testing.T) {
	t.Parallel()
	a := assert.New(t)
	a.Implements((*goat.Provider)(nil), provider())
}
...
func Test_FetchUser(t *testing.T) {
	...
	for _, testData := range []struct {
		...
		session      goat.Session
		handler      http.Handler
		expectedUser goat.User
		expectErr    bool
	}{
		{
			...
			handler: http.HandlerFunc(
				func(w http.ResponseWriter, r *http.Request) {
					...
					case "/api/auth.test":
						...
						json.NewEncoder(w).Encode(testAuthTestResponseData)
					case "/api/users.info":
						w.WriteHeader(http.StatusOK)
						json.NewEncoder(w).Encode(testUserInfoResponseData)
					default:
...
func Test_SessionFromJSON(t *testing.T) {
	...
	p := provider()
	session, err := p.UnmarshalSession(
		`{"AuthURL":"https://slack.com/oauth/authorize","AccessToken":"1234567890"}`,
	)
	a.NoError(err)
